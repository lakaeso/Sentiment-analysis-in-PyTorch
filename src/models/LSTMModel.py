...
class LSTMModel(nn.Module, AbstractModel):
    """NLP model based on RNNs."""
    ...
    def __init__(self, config: dict) -> None:
        ...
        # network
        self.lstm1 = nn.LSTM(300, 150, 2, batch_first=False)
        self.lstm2 = nn.LSTM(150, 150, 2, batch_first=False)
    ...
    def train_(self, verbose=False):
        ...
        for i_epoch in range(1, self.num_epoch + 1):
            ...
            if verbose:
                print(f"... LSTM model starting epoch {i_epoch}")
            ...
            if verbose:
                print(f"... LSTM model epoch {i_epoch}, eval acc={eval_acc:.2f}")
        
        if verbose:
            print(f"... LSTM model finished training, test acc={test_acc:.2f}")
    ...
    def forward(self, x):
        ...
        h, hidden = self.lstm1(h)
        h, hidden = self.lstm2(h, hidden)
        h = self.fc1(h[-1])
        h = torch.relu(h)
        h = self.fc2(h)
    ...
    def get_parameters(self):
        ...
        parameters = []
        parameters.extend(list(self.lstm1.parameters()))
        parameters.extend(list(self.lstm2.parameters()))
        parameters.extend(list(self.fc1.parameters()))
        parameters.extend(list(self.fc2.parameters()))
        parameters.extend(list(self.embedding.parameters()))
